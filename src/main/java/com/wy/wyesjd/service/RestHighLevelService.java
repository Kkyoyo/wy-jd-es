...
public class RestHighLevelService {
    ...
    public  void testMonet(String indexNames) throws IOException {
        SearchRequest request = new SearchRequest(indexNames);
        SearchSourceBuilder sourceBuilder = new SearchSourceBuilder();
        AggregationBuilder aggregationBuilder = AggregationBuilders.terms("ORIG").field("PNR_BOOKDATE.keyword").size(10)
                .subAggregation(AggregationBuilders.terms("two").field("TRIP_ORIG.keyword").size(10)
                        .subAggregation(AggregationBuilders.terms("3").field("SALDATE").size(8))
                );
        sourceBuilder.aggregation(aggregationBuilder);
        sourceBuilder.size(0);
        request.source(sourceBuilder);
